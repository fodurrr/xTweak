# Codex Agent Usage Guide (Updated October 27, 2025)
...
**Examples:**
```bash
# Launch MCP verification agent
CODE_CONFIG_HOME=$(pwd)/scripts/codex/local codex --profile xtweak-mcp-verify-first "Verify current project state and list all Ash resources"

# Launch Ash resource architect
CODE_CONFIG_HOME=$(pwd)/scripts/codex/local codex --profile xtweak-ash-resource-architect "Add Comment resource with user relationship"

# Launch frontend design enforcer
CODE_CONFIG_HOME=$(pwd)/scripts/codex/local codex --profile xtweak-frontend-design-enforcer "Implement user profile LiveView page"

# Launch test builder
CODE_CONFIG_HOME=$(pwd)/scripts/codex/local codex --profile xtweak-test-builder "Write tests for Newsletter resource"
```
