defmodule XTweakWebWeb.Components.UI.ThemeSwitcher do
  @moduledoc """
  ...
  attr :live_view, :boolean, default: true, doc: "Whether this is used in a LiveView context"
  ...
  For static pages, uses JavaScript events instead of Phoenix hooks.
  """
  def theme_switcher(assigns) do
    ...
    <div
      ...
      phx-hook={if @live_view, do: "UniversalTheme", else: nil}
      data-storage-key="xpando-theme-preference"
      data-default-theme="dark"
      data-is-static={if !@live_view, do: "true", else: nil}
    >
      <div
        tabindex="0"
        role="button"
        ...
        <%= for theme <- available_themes() do %>
          <li role="none">
            <button
              ...
              phx-click={if @live_view, do: apply_theme(theme.value, @id), else: nil}
              onclick={
                if !@live_view,
                  do:
                    "window.dispatchEvent(new CustomEvent('xpando:theme-change', { detail: { theme: '#{theme.value}' } })); document.getElementById('#{@id}').classList.remove('dropdown-open')",
                  else: nil
              }
              role="menuitem"
