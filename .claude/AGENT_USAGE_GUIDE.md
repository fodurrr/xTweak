...
| `nuxt-ui-expert` | Read-only Nuxt UI component API research | Component specs (props/slots/events), markdown + JSON output, accessibility docs | `frontend-design-enforcer` |
...
- **LiveView UI**: `frontend-design-enforcer` orchestrates builds; pull component/theming support from `daisyui-expert` or `nuxt-ui-expert` and utility/layout strategy from `tailwind-strategist`.
